...
import { didPlayerEstimateCorrectly } from './estimationResultUtils';
...
export function EstimationGameScreen({
  ...
  const estimatedCorrectly = didPlayerEstimateCorrectly(room.estimationRoundResult, playerId);
  ...
  useEffect(() => {
    if (room.status === 'result' && estimatedCorrectly) {
      confetti({ particleCount: 55, spread: 75, origin: { y: 0.62 }, scalar: 0.75 });
    }
  }, [room.status, room.currentRound, estimatedCorrectly]);

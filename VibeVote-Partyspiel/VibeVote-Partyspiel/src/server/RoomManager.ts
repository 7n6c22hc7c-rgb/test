...
export class RoomManager extends EventEmitter {
  ...
  private startEstimationQuestion(room: InternalRoom): void {
    ...
    room.roundResult = undefined;
    room.deadline = Date.now() + room.estimationSettings.timerSeconds * 1_000;
    room.voteTimer = setTimeout(() => {
      if (room.status === 'voting' && room.selectedGame === 'estimation') {
        this.revealEstimationRoom(room, true);
      }
    }, room.estimationSettings.timerSeconds * 1_000);
    room.voteTimer.unref?.();
    this.emitChanged(room.code);
  }
  ...
  private revealEstimationRoom(room: InternalRoom, timerExpired = false): void {
    this.requireStatus(room, 'voting');
    this.requireSelectedGame(room, 'estimation');
    const activePlayers = this.connectedPlayers(room);
    ...
    const deadlineReached = room.deadline !== undefined && Date.now() >= room.deadline;
    if (!timerExpired && !allAnswered && !deadlineReached) {
      throw new PartyGameError('WAITING_FOR_ANSWERS', 'Das Ergebnis erscheint, sobald alle aktiven Spieler geantwortet haben oder die Zeit abgelaufen ist.');
    }

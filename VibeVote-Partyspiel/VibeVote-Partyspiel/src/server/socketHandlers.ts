...
export function registerSocketHandlers(io: Server, roomManager: RoomManager): void {
  roomManager.on('changed', (roomCode: string) => {
    ...
    for (const socketId of socketIds) {
      const snapshot = roomManager.getSnapshotForSocket(socketId);
      if (snapshot) io.to(socketId).emit('room:state', snapshot);
    }
  });
  ...
  io.on('connection', (socket: Socket) => {
    ...
    socket.on('room:select-game', (payload: { gameId?: unknown }, callback: AckCallback) => {
      handle(callback, () => roomManager.selectGame(socket.id, payload?.gameId));
    });
    socket.on('estimation:update-settings', (payload: Partial<EstimationSettings>, callback: AckCallback) => {
      handle(callback, () => roomManager.updateEstimationSettings(socket.id, payload ?? {}));
    });
    ...
    socket.on('estimation:submit-answer', (payload: { answer: unknown }, callback: AckCallback) => {
      handle(callback, () => roomManager.submitEstimationAnswer(socket.id, payload?.answer));
    });

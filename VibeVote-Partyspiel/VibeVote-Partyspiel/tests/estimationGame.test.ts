...
describe('Schätzfragen-Pool', () => {
  it('enthält 320 eindeutige, ausgewogen verteilte und valide Fragen', () => {
    expect(estimationQuestions).toHaveLength(320);
    expect(new Set(estimationQuestions.map((question) => question.id)).size).toBe(320);
    expect(new Set(estimationQuestions.map((question) => question.text)).size).toBe(320);

    const expectedDistribution = {
      Allgemeinwissen: { total: 54, Leicht: 19, Mittel: 18, Schwer: 17 },
      Geographie: { total: 54, Leicht: 19, Mittel: 18, Schwer: 17 },
      Geschichte: { total: 53, Leicht: 18, Mittel: 18, Schwer: 17 },
      Sport: { total: 53, Leicht: 18, Mittel: 18, Schwer: 17 },
      Natur: { total: 53, Leicht: 18, Mittel: 18, Schwer: 17 },
      Kultur: { total: 53, Leicht: 18, Mittel: 18, Schwer: 17 },
    } as const;
    ...
    for (const category of ESTIMATION_CATEGORIES) {
      ...
      expect(categoryQuestions).toHaveLength(expectedDistribution[category].total);
      expect(categoryQuestions.filter((question) => question.difficulty === 'Leicht'))
        .toHaveLength(expectedDistribution[category].Leicht);
      expect(categoryQuestions.filter((question) => question.difficulty === 'Mittel'))
        .toHaveLength(expectedDistribution[category].Mittel);
      expect(categoryQuestions.filter((question) => question.difficulty === 'Schwer'))
        .toHaveLength(expectedDistribution[category].Schwer);
    }
...
describe('Schätzfragen-Einstellungen und Eingaben', () => {
  ...
  it('begrenzt die Rundenzahl automatisch auf den verfügbaren Pool', () => {
    ...
    expect(countEstimationQuestions(validation.value)).toBe(17);
    expect(validation.value.roundLimit).toBe(17);
  });

...
export function GameScreen({ room, playerId, onVote, onReveal, onNext, onEnd, onLeave }: GameScreenProps) {
        ...
        {isHost ? (
          <div className="game-actions">
            ...
            <button type="button" className="primary-button large-button" onClick={() => void onNext()}>
              {room.remainingQuestions === 0 ? 'Zur Auswertung' : 'Nächste Frage'} <ArrowRight size={20} />
            </button>
          </div>
        ) : (
          <div className="waiting-host"><span className="waiting-dots"><i /><i /><i /></span>Der Host startet gleich die nächste Frage.</div>
        )}
    ...
    <div className="game-layout page-enter">
      ...
      {!alreadyVoted ? (
        ...
        <section className="waiting-card glass-card vote-confirmed">
          ...
          <p>Deine Wahl bleibt geheim. Warte auf die anderen Spieler.</p>
          ...
          {isHost && (
            <button
              ...
              disabled={!room.canHostReveal}
              onClick={() => void onReveal()}
            >
              <Medal size={19} /> {room.canHostReveal ? 'Ergebnis für alle anzeigen' : 'Warte auf alle Stimmen'}
            </button>
          )}
        </section>
      )}
      ...
      {!alreadyVoted && isHost && (
        <button
          type="button"
          className="secondary-button centered-button"
          disabled={!room.canHostReveal}
          onClick={() => void onReveal()}
        >
          <Medal size={18} /> Ergebnis anzeigen
        </button>
      )}

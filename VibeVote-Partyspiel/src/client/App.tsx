...
import { EstimationFinishedScreen } from './games/estimation/EstimationFinishedScreen';
import { EstimationGameScreen } from './games/estimation/EstimationGameScreen';
  ...
  if (!identity || !room) {
    ...
    content = (
      <LobbyScreen
        ...
        onUpdateEstimationSettings={(settings: Partial<EstimationSettings>) => action('estimation:update-settings', settings)}
        onSelectGame={(gameId) => action('room:select-game', { gameId })}
    ...
    content = room.selectedGame === 'estimation' ? (
      <EstimationFinishedScreen
        room={room}
        playerId={identity.playerId}
    ...
    content = room.selectedGame === 'estimation' ? (
      <EstimationGameScreen
        room={room}
        playerId={identity.playerId}
        onSubmitAnswer={(answer: EstimationAnswer) => action('estimation:submit-answer', { answer })}

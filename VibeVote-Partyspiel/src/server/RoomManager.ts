...
export class RoomManager extends EventEmitter {
  ...
  updateSettings(socketId: string, update: Partial<RoomSettings>): void {
    ...
    if (![10, 20, 30, 'all'].includes(nextSettings.roundLimit)) {
      throw new PartyGameError('INVALID_SETTINGS', 'Die gewählte Rundenzahl ist ungültig.');
    }
    if (![0, 20, 25, 30].includes(nextSettings.timerSeconds)) {
      throw new PartyGameError('INVALID_SETTINGS', 'Der gewählte Timer ist ungültig.');
    }
    if (!['drink', 'point', 'task'].includes(nextSettings.playMode)) {
      throw new PartyGameError('INVALID_SETTINGS', 'Der gewählte Spielmodus ist ungültig.');
    }
  ...
  private beginNewGame(room: InternalRoom): void {
    this.resetGame(room);
    this.resetPlayerStats(room);
    const requestedRounds = room.settings.roundLimit === 'all'
      ? questions.length
      : room.settings.roundLimit;
    room.questionOrder = shuffle(questions, this.random).slice(0, requestedRounds);
    room.currentIndex = 0;
  ...
  private startVoting(room: InternalRoom): void {
    ...
    room.deadline = room.settings.timerSeconds > 0
      ? Date.now() + room.settings.timerSeconds * 1_000
      : undefined;

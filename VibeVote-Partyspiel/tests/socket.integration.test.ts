...
describe('Socket.IO Multiplayer-Ablauf', () => {
  ...
  it('synchronisiert Lobby, Abstimmung und gemeinsames Ergebnis', async () => {
    ...
    const guestVoteState = nextRoomState(guestClient, (room) => room.votesSubmitted === 2);
    ...
    const allVoted = await guestVoteState;
    expect(allVoted.allActivePlayersVoted).toBe(true);
    ...
    const revealAck = await emitAck<void>(hostClient, 'game:reveal');

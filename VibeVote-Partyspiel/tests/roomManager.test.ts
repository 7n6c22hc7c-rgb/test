...
describe('RoomManager', () => {
  ...
  it('begrenzt eine Partie auf maximal 60 Fragen und bietet einen 10-Sekunden-Timer', () => {
    vi.useFakeTimers();
    const manager = new RoomManager();
    const host = manager.createRoom('Host', 'socket-host');
    manager.joinRoom(host.roomCode, 'Gast', 'socket-guest');
    manager.updateSettings('socket-host', { roundLimit: 60, timerSeconds: 10 });
    manager.startGame('socket-host', true);

    const snapshot = manager.getSnapshot(host.roomCode)!;
    expect(snapshot.totalRounds).toBe(60);
    expect(snapshot.settings.timerSeconds).toBe(10);
    expect(snapshot.deadline).toBe(Date.now() + 10_000);
    manager.destroy();
  });
  ...
  it('zeigt nach der letzten Stimme automatisch das Ergebnis und wartet auf den Host', () => {
    vi.useFakeTimers();
    const manager = new RoomManager();
    const host = manager.createRoom('Host', 'socket-host');
    const guest = manager.joinRoom(host.roomCode, 'Gast', 'socket-guest');
    ...
    manager.nextQuestion('socket-host');
    const nextQuestion = manager.getSnapshot(host.roomCode)!;
    expect(nextQuestion.status).toBe('voting');
    expect(nextQuestion.currentRound).toBe(2);
    manager.destroy();
  });
